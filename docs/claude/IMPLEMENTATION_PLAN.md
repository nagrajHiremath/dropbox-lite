...
**Implemented centrally at `api-gateway`**, not as a hand-rolled limiter duplicated across upload/download/metadata/account-service. Every one of the 5 priority targets is already a gateway route, keyed by `userKeyResolver` (reads the already-gateway-set `X-User-Id` header - upload/download/metadata) or `ipKeyResolver` (remote address - public share/auth, both pre-authentication).

Originally used Spring Cloud Gateway's own built-in `RequestRateLimiter` filter + `RedisRateLimiter` (token bucket, atomic Lua script bundled with Gateway itself). Since replaced with a custom `com.dropbox.api_gateway.ratelimit` package - four independent Redis-backed `RateLimiter` implementations (fixed window, sliding window via ZSet, sliding window via Lua, token bucket via Lua), each `@ConditionalOnProperty(app.rate-limit.method=...)` so exactly one is active at a time (default `sliding-lua` - atomic single-script check, no fixed-window edge-burst). A `CustomRateLimiterGatewayFilterFactory` (filter name `CustomRateLimiter` in `application.yaml`) wires the active `RateLimiter` bean into each route, reusing the same `userKeyResolver`/`ipKeyResolver` beans unchanged. All 7 routes keep the same predicates/uri/priority tiers, now tuned via `(max-requests, window-seconds)` instead of `(replenishRate, burstCapacity)`. `Retry-After` is now set directly from the exact value `RateLimitResult` computes per algorithm, so the earlier `RetryAfterHeaderFilter` workaround (needed only because `RedisRateLimiter` never exposed a real retry time) was deleted.

**Verified live end-to-end**: booted api-gateway locally against real Redis and burst-tested the auth route (`max-requests=5`, `window-seconds=1`, `sliding-lua`) with 5 rapid requests succeeding and the 6th correctly rejected with `429`, an exact `Retry-After: 1`, and `X-RateLimit-Remaining: 0`. Two real bugs were caught and fixed during the original built-in-filter implementation (not by code review alone), and both lessons still apply to the custom filter since it reuses the same pieces: a `NoUniqueBeanDefinitionException` from having two `KeyResolver` beans with no `@Primary` (any Gateway filter factory needs an unambiguous default even though every route here sets `key-resolver` explicitly), and the original `Retry-After` implementation (`chain.filter(exchange).then(...)`) never running on a rejected request because `RequestRateLimiter` short-circuits without calling `chain.filter()` further - the custom filter sidesteps this entirely by setting the header inline before completing the response itself, rather than needing a separate ordering-independent global filter.

A third bug surfaced after deploy, from real multi-file-upload usage no longer tripping `429` the way it used to: none of the four `RateLimiter` implementations scope their Redis key by route, only by the resolved `KeyResolver` value (user id or IP) - so `upload-service` (max 10), `download-file-content` (max 20), and `metadata-service` (max 40), all on `userKeyResolver`, were silently sharing one Redis counter per user instead of one per route, each route's check comparing that shared count against its own threshold. The old built-in `RedisRateLimiter` didn't have this problem - it already scoped its key by route id (`request_rate_limiter.{routeId}.{key}.*`, per the original verification note above). Fixed by scoping the key in `CustomRateLimiterGatewayFilterFactory` itself, before it ever reaches a `RateLimiter`: `{route.getId()}:{resolvedKey}`, read from the standard `ServerWebExchangeUtils.GATEWAY_ROUTE_ATTR` exchange attribute. Re-verified live: exhausted the auth route's 5-request limit, then immediately confirmed the public-share route (same `ipKeyResolver`, same client IP, different route) still had its own fresh 5 requests rather than being pre-blocked by auth's exhausted counter.
